...
#+BEGIN_SRC sh
  ...
  {
      if [[ $(acpi 2&>/dev/null | grep -c '^Battery.*') -eq 0 ]] ; then
          return
      fi
      b=""
      if [[ $(acpi 2&>/dev/null | grep -c '^Battery.*Full') -gt 0 ]] ; then
          b=" ⌁ "
      elif [[ $(acpi 2&>/dev/null | grep -c '^Battery.*harging') -gt 0 ]] ; then
          b=$(acpi | cut -f2 -d ',' | tr -cd '[:digit:]')
          if [ $b -lt 10 ] ; then
              POWERLINE_COLOR_BG_BATTERY=$bg[red]
              POWERLINE_COLOR_FG_BATTERY=$fg[red]
          fi
      elif [[ $(acpi 2&>/dev/null | grep -c '^Battery.*Charging') -gt 0 ]] ; then
          b+=" ⌁ "
          b+=$(acpi | cut -f2 -d ',' | tr -cd '[:digit:]')
      fi
